...
doc = Document()
doc.add_heading('Data Analysis and Model Performance Report', 0)  # Level 0 for the title

# Adding a paragraph with an explicitly named style
p = doc.add_paragraph(f'Accuracy of the model: {accuracy:.2f}')
p.style = 'Normal'  # Explicitly setting the style to 'Normal'
...
"""
doc.add_paragraph(confusion_matrix_description, style='Normal')  # Explicit style name

confusion_matrix_path = output_dir / 'confusion_matrix.png'
plt.savefig(confusion_matrix_path)
doc.add_heading('Confusion Matrix:', level=2)  # Using level 2 for subheadings
...
"""
doc.add_paragraph(feature_importances_description, style='Normal')  # Explicit style name

feature_importances_path = output_dir / 'feature_importances.png'
plt.savefig(feature_importances_path)
doc.add_heading('Feature Importances:', level=2)  # Using level 2 for subheadings
...
doc.add_heading('Crucial Job Titles to Hire', level=1)  # Using level 1 for main headings
doc.add_paragraph('Based on our analysis, the following job titles are in high demand and crucial to hire:', style='Normal')
for job_title, count in most_demanded_job_titles.items():
    doc.add_paragraph(f'{job_title}: {count} listings', style='List Bullet')  # Corrected style name

# Recommendations for HR manager
doc.add_heading('HR Manager Recommendations', level=1)  # Using level 1 for main headings
recommendations = """
...
"""
doc.add_paragraph(recommendations, style='Normal')  # Explicit style name
